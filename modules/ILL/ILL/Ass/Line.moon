...
class Line
	...
	extend: (ass, l, i, noblank) ->
		Line.process ass, l
		unless l.isShape
			l.lines = Line.lineBreaks ass, l, noblank
			if jit.os != "Windows"
				-- gets the values from the bounding boxes provided by the SubInspector module
				realBound = ass.bounds[i]
				realWidth = realBound.r - realBound.l
				realHeight = realBound.b - realBound.t
				-- resets the perspective values of the new line
				lcopy = Table.copy l
				for lineBreak in *lcopy.lines
					for lineBlock in *lineBreak
						with lineBlock.data
							...
							.scale_x = 100
							.scale_y = 100
				-- gets the delimiter box from the text generated by the pangocairo library
				ll, tt, rr, bb = nil, nil, 0, 0
				Line.callBackExpand ass, lcopy, nil, (line) ->
					{px, py} = line.data.pos
					{l: left, t: top, r: right, b: bottom} = Path(line.shape)\move(px, py)\boundingBox!
					ll = math.min ll or left, left
					tt = math.min tt or top, top
					rr = math.max rr, right
					bb = math.max bb, bottom
				aegiWidth = rr - ll
				aegiHeight = bb - tt
				-- gets the ratio between the two bounding boxes
				areaA = realWidth ^ 2
				areaB = aegiWidth ^ 2
				ratio = 1
				if areaA > areaB
