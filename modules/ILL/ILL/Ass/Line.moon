...
class Line
	...
	callBackFBF: (ass, l, fn) ->
		...
		-- gets the offset between one frame and another
		off_frame = math.floor (Aegi.mff(stt_frame) - Aegi.mff(stt_frame + 1)) / 2
		-- iterates over all the identified frames
		for i = stt_frame + 1, end_frame
			s = Aegi.mff i - 1
			e = Aegi.mff i
			c = s - off_frame - start_time
			...
			line.text\callBack (tags, text, j) ->
				if j == 1
					lerpTagMove c, dado, tags
				lerpTagTransform c, dado, tags
				lerpTagFade c, dado, tags
				return tags, text
			fn line, i, end_frame
